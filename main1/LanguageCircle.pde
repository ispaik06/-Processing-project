  ...
  void openLanguageWindow() {
    if (!isCardVisible) {
      ...
      if (row != null) {
        ...
        card = new Card(cardX, cardY, row.getString("Name"), row.getString("Year"), row.getString("Developer"), row.getString("Purpose"), row.getString("Significance"), getImageForLanguage(name), startColor);
        isCardVisible = true;
      }
    }
  }
